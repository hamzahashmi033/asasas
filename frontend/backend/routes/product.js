...
router.post("/add", auth.authenticate, role.checkRole, (req, res) => {
    ...
    let query =
        'insert into product (name, category, description, price, status) values(?,?,?,?,"true")';
...
router.get("/get", auth.authenticate, (req, res, next) => {
    let query =
        "select p.id, p.name, p.description, p.price, p.status, c.id as categoryID, c.name as categoryName from product as p INNER JOIN category as c where p.category=c.id ";
...
router.get("/getByCategoryID/:id", auth.authenticate, (req, res, next) => {
    ...
    let query =
        'select id, name from product where category=? and status="true"';
...
router.patch("/update", auth.authenticate, role.checkRole, (req, res, next) => {
    let product = req.body;
    let query =
        "update product set name=?, category=?, description=?, price=? where id=?";
    connection.query(
        query,
        [
            product.name,
